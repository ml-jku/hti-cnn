...
class Cellpainting_IG(TorchDataset):
    def __init__(self, sample_index_file: str, data_directory_path: str, label_matrix_file: str = None,
                 label_row_index_file: str = None, label_col_index_file: str = None, auxiliary_labels=None,
                 transforms=None, group_views: bool = False,
                 subset: float = 1., num_classes: int = None, verbose: bool = False):
...
if __name__ == '__main__':
    # basedir = r"C:\Users\Markus\Projects\cellpainting"
    basedir = "/local00/bioinf/markus/cellpainting/"
    data_dir = path.join(basedir, "npzs/val")
    # label_dir = path.join(basedir, "data")
    label_dir = path.join(basedir, "chembl_label_matrix")
    # data_dir = r'/local00/bioinf/cellpainting/npzs/val'
    # label_dir = r"/publicdata/cellpainting/chembl_label_matrix"
    label_matrix_file = path.join(label_dir, "cellpainting-label-matrix.mtx")
    label_row_index_file = path.join(label_dir, "cellpainting-compounds.csv")
    label_col_index_file = path.join(label_dir, "cellpainting-assays.csv")
    sample_index_file = path.join(label_dir, "cellpainting-val.csv")
    data_file = path.join(basedir, "cellprofiler-new-features-well.npz")
    # --
    # transforms = Compose([ToTensor(), NormalizeByImage()])
    # auxiliary_label_file = r"C:\Users\Markus\Projects\cellpainting\cellpainting_predictions.csv"
    auxiliary_label_file = None

    if False:
        #transforms = Compose(
        #    [invoke_functional_with_params("torchvision.transforms.ToTensor()"), invoke_functional_with_params("pyll.transforms.NormalizeByImage()")])
        transforms = Compose([invoke_functional_with_params("torchvision.transforms.ToTensor()")])

        cellpainting = Cellpainting(data_directory_path=data_dir, label_matrix_file=label_matrix_file,
                                    label_row_index_file=label_row_index_file, label_col_index_file=label_col_index_file,
                                    sample_index_file=sample_index_file, auxiliary_labels=auxiliary_label_file, transforms=transforms, group_views=False,
                                    verbose=False)

    if True:
        cellpainting = CellpaintingPrecalculated(data_file=data_file, label_matrix_file=label_matrix_file,
                                                 label_row_index_file=label_row_index_file, label_col_index_file=label_col_index_file,
                                                 sample_index_file=sample_index_file, auxiliary_labels=auxiliary_label_file, verbose=False)
    ...
    with open("/publicwork/hofmarcher/cellpainting/sample_order_new_features_bs{}_j{}.csv".format(batchsize, num_workers), "w") as f:
        duration = 0
        t = Timer("Read sample", verbose=False)
        for i, batch in enumerate(loader):
            duration += t.stop()
            ids = batch["ID"]
            for id in ids:
                f.write("{}\n".format(id))
            print("{}/{} ({}ms)".format(i+1, len(loader), duration / (i+1)), end="\r")
            t.restart()
    print("{}/{} ({}ms)".format(i+1, len(loader), duration / (i + 1)), end="\n")
